...
def get_pet_labels(image_dir):
    ...
    # Process each file to create the pet label
    for filename in filename_list:
        if filename[0] != ".":  # Skip hidden files
            # Convert to lowercase and split by "_"
            label_words = filename.lower().split("_")
            
            # Filter alphabetic words and join them into a label
            pet_label = " ".join([word for word in label_words if word.isalpha()])
            pet_label = pet_label.strip()
            
            # Add to the dictionary (key: filename, value: [pet_label])
